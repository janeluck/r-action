...
(function ($) {
    ...
    $.extend($.jqx._jqxSlider.prototype, {
        ...
        _addTicks: function (container) {
            ...
            container.empty();
            var ticks = "";
            var htmlValue = this.tickLabelFormatFunction ? this.tickLabelFormatFunction(this.min) : this.min;
            ...
            for (var i = 1; i < tickscount; i++) {
                ...
                var value = this.min + this.ticksFrequency * i;
            ...
            var htmlValue = this.tickLabelFormatFunction ? this.tickLabelFormatFunction(this.max) : this.max;
            ticks += this._addTick(container, tickscount * distance, this.max, size, htmlValue, spanSize);
            container.append($(ticks));
        },

        _addTick: function (container, position, value, size, htmlValue, spanSize) {
            var cssClass = "";
            ...
            if (this.orientation === 'horizontal') {
                ...
                if (this.showTickLabels) {
                    if (container[0] !== this._topTicks[0]) {
                        topValue = size - this.tickSize - spanSize.height - 2 + 'px';
                    }
                    else {
                        topValue = 2 + this.tickSize + 'px';
                    }
                ...
                if (this.showTickLabels) {
                    if (container[0] !== this._topTicks[0]) {
                        topValue = size - this.tickSize - htmlValue.toString().length * spanSize.width - 6 + 'px';
                    }
                    else {
                        topValue = 6 + this.tickSize + 'px';
                    }
        ...
        _getFixedValueByPosition: function (position) {
            ...
            var thumbSize = this._getThumbSize();
            var step = this.step,
                count = (this.max - this.min) / step, sectorSize = (trackSize) / count,
