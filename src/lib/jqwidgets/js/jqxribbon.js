    ...
    $.extend($.jqx._jqxRibbon.prototype, {
        ...
        addAt: function (index, data) {
            ...
            var newItemContent = $('<div class="' + me.toThemeProperty('jqx-widget-content') + ' ' + me.toThemeProperty('jqx-ribbon-content-section') + ' ' +
                me.toThemeProperty('jqx-ribbon-content-section-' + me.position) + '">' + data.content + '</div>');
            switch (me.position) {
                case 'top':
        ...
        // selects an item
        _selectAt: function (index, oldIndex) {
            var me = this;
        ...
        _addHandlers: function () {
            ...
            if (me.selectionMode === 'click') {
                me.addHandler(me._items, 'click.ribbon' + me.element.id, function (event) {
                    ...
                    if (!me._items[index]._disabled) {
                        if (index !== me.selectedIndex) {
                            ...
                            if (me.popupCloseMode != 'none') {
                                ...
                                $(event.target).addClass(me.toThemeProperty('jqx-ribbon-item-hover'));
                                me._clearSelection();
                            }
                        }
                    }
                ...
                me.addHandler(me._items, 'mouseenter.ribbon' + me.element.id, function (event) {
                    if (condition(event.target._index)) {
                        $(event.target).addClass(me.toThemeProperty('jqx-fill-state-hover'));
                        $(event.target).addClass(me.toThemeProperty('jqx-ribbon-item-hover'));
                    }
                });

                me.addHandler(me._items, 'mouseleave.ribbon' + me.element.id, function (event) {
                    if (condition(event.target._index)) {
                        $(event.target).removeClass(me.toThemeProperty('jqx-fill-state-hover'));
                        $(event.target).removeClass(me.toThemeProperty('jqx-ribbon-item-hover'));
                    }
                });
                if (me.mode === 'popup') {
                    me.addHandler(me.host, 'mouseleave.ribbon' + me.element.id, function () {
                        if (me.popupCloseMode == "mouseLeave" && me.mode === 'popup') {
                            me._clearSelection();
                        }
                    });
                    me.addHandler(me._contentSections, 'mouseleave.ribbon' + me.element.id, function () {
                        if (me.popupCloseMode == "mouseLeave" && me.mode === 'popup') {
                            me._clearSelection();
                        }
                    });
                    me.addHandler($(document), 'mousedown.ribbon' + me.element.id, function (event) {
                        closeOnClick(event);
                    });
                }
            ...
            me.addHandler(scrollButtonNear, 'mousedown.ribbon' + me.element.id, function () {
                if (me._orientation === 'horizontal') {
            ...
            me.addHandler(scrollButtonNear, 'mouseup.ribbon' + me.element.id, function () {
                clearInterval(me._timeoutNear);
            });

            var scrollButtonFar = $(me._scrollButtons[1]);
            me.addHandler(scrollButtonFar, 'mousedown.ribbon' + me.element.id, function () {
                if (me._orientation === 'horizontal') {
            ...
            me.addHandler(scrollButtonFar, 'mouseup.ribbon' + me.element.id, function () {
        ...
        _removeHandlers: function (selectionMode, mode) {
            ...
            if (selectionMode === 'click') {
                me.removeHandler(me._items, 'click.ribbon' + me.element.id);
                me.removeHandler(me._items, 'mouseenter.ribbon' + me.element.id);
                me.removeHandler(me._items, 'mouseleave.ribbon' + me.element.id);
            } else if (selectionMode === 'hover') {
                me.removeHandler(me._items, 'mouseenter.ribbon' + me.element.id);
                if (mode === 'popup') {
                    me.removeHandler(me.host, 'mouseleave.ribbon' + me.element.id);
                }
            }

            var scrollButtonNear = $(me._scrollButtons[0]);
            me.removeHandler(scrollButtonNear, 'mousedown.ribbon' + me.element.id);
            me.removeHandler(scrollButtonNear, 'mouseup.ribbon' + me.element.id);
            var scrollButtonFar = $(me._scrollButtons[1]);
            me.removeHandler(scrollButtonFar, 'mousedown.ribbon' + me.element.id);
            me.removeHandler(scrollButtonFar, 'mouseup.ribbon' + me.element.id);
        },
        ...
        _addClasses: function () {
            ...
            me._header.removeClass();
            me._items.removeClass();
            me.host.removeClass();
        ...
        _appendScrollButtons: function () {
            ...
            var arrowDirection = (me._orientation === 'horizontal') ? ['left', 'right'] : ['top', 'bottom'];
        ...
        _animationComplete: function (index, oldIndex) {
            ...
            var unselectedIndex = oldIndex !== -1 ? oldIndex : null;
