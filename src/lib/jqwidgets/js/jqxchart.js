...
(function ($) {
    ...
    $.jqx.svgRenderer.prototype = {
        ...
        init: function (host) {
            var s = "<table id=tblChart cellspacing='0' cellpadding='0' border='0' align='left' valign='top'><tr><td colspan=2 id=tdTop></td></tr><tr><td id=tdLeft></td><td><div class='chartContainer' onselectstart='return false;'></div></td></tr></table>";
            host.append(s);
            this.host = host;
    ...
    ************************************************/
    ...
    /** @private */
    $.jqx._widgetToImage = function (widgetInstance, type, fileName, exportServer, isUploadOnly) {
        var self = widgetInstance;
        ...
        var canvas = self.renderer.getContainer()[0];

        var result = $.jqx.exportImage(canvas, type, fileName, exportServer, isUploadOnly);
    ...
    $.extend($.jqx._jqxChart.prototype,
    {
        ...
        _renderChartLegend: function (data, rect, isMeasure, isVerticalFlow) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if (sz.width > rect.width) {
                    ...
                    var wrapText = "";
                    var textInfo = [];
                    while (undefined != (word = words.pop())) {
                        line.push(word);
                        wrapText = line.join(" ");
        ...
        _renderXAxis: function (groupIndex, rect, isMeasure, chartRect) {
            ...
            if (axis.type == 'date') {
                ...
                settings.gridLines.offsets = this._generateOffsets(min, max, axisSize, padding, gridLinesInterval, ui, valuesOnTicks, NaN, false, flip);
                settings.tickMarks.offsets = this._generateOffsets(min, max, axisSize, padding, tickMarksInterval, ui, valuesOnTicks, NaN, false, flip);

                labelOffsets = this._generateOffsets(min, max, axisSize, padding, labelsInterval, ui, valuesOnTicks, NaN, true, flip);
            }
        ...
        _renderSpiderAxis: function (groupIndex, rect) {
            ...
            if (axis.type == 'date') {
                ...
                gridLinesSettings.offsets = this._generateOffsets(min, max, axisSize, padding, gridLinesInterval, ui, true, 0, false, flip);
                tickMarksSettings.offsets = this._generateOffsets(min, max, axisSize, padding, tickMarksInterval, ui, true, 0, false, flip);
                labelOffsets = this._generateOffsets(min, max, axisSize, padding, labelsInterval, ui, true, 0, false, flip);
            }
            ...
            if (group.spider) {
                ...
                if (axis.type == 'date')
                    ...
                    spiderOffsets = this._generateOffsets(min, max, axisSize, padding, ui, ui, true, 0, false, flip);
        ...
        _renderSpiderValueAxis: function (groupIndex, rect, radius, offsetAngles) {
            ...
            if (items.length && settings.line.visible) {
                var x2 = offsetX + Math.cos(startAngle) * radius;
                var y2 = offsetY + Math.sin(startAngle) * radius;

                if (offsetAngles.indexOf(startAngle) == -1) {
            ...
            if (gridLines.visible || group.spider) {
                var strokeAttributes = { stroke: gridLines.color, fill: 'none', 'stroke-width': 1, 'stroke-dasharray': gridLines.dashStyle || '' };
                for (var i = 0; i < len; i += gridLinesInterval) {
            ...
            if (tickMarks.visible) {
                ...
                for (var i = 0; i < len; i += tickMarksInterval) {
                    if (gridLines.visible && (i % gridLinesInterval) == 0)
                        continue;
        ...
        _renderAxis: function (isVertical, isMirror, axisSettings, rect, chartRect, ui, isLogAxis, valuesOnTicks, itemsInfo, isMeasure, animationDuration) {
            ...
            for (var i = 0; i < items.length && textSettings.visible; i++, offset += itemWidth) {
                ...
                if (!isMeasure) {
                    ...
                    items[i].visible = true; //!isLogAxis || (isLogAxis && (i % ui) == 0);
                }
            }
        ...
        _drawPlotAreaLines: function (groupIndex, isValueAxis, itemsToDraw) {
            ...
            if (itemsToDraw.tickMarks && tickMarksSettings.visible && state.tickMarks != true) {
                ...
                for (var i = 0; i < offsets.length; i++) {
                    var lineOffset = $.jqx._ptrnd((swapXY ? rect.y + offsets[i].offset : rect.x + offsets[i].offset));
        ...
        _renderValueAxis: function (groupIndex, rect, isMeasure, chartRect) {
            ...
            for (var i = 0; i < renderData.items.length; i++) {
                ...
                var text = (labelsSettings.formatFunction) ? labelsSettings.formatFunction(value) : this._formatNumber(value, formatSettings);
            ...
            if (gridLines.visible || axis.alternatingBackgroundColor || axis.alternatingBackgroundColor2) {
                gridLines.offsets = this._generateOffsets(min, max, axisSize, { left: 0, right: 0 }, gridLinesInterval, mu, true, 0, false, !flip);
                if (logAxis && !isNaN(gridLines.step)) {
                    var arr = [];
                    for (var i = 0; i < gridLines.offsets.length; i += gridLines.step)
                        arr.push(gridLines.offsets[i]);

                    gridLines.offsets = arr;
                }
            }

            var tickMarks = axisSettings.tickMarks;
            var tickMarksInterval = logAxis ? mu : this._getInterval(tickMarks, mu);

            if (tickMarks.visible) {
                tickMarks.offsets = this._generateOffsets(min, max, axisSize, { left: 0, right: 0 }, tickMarksInterval, mu, true, 0, false, !flip);
                if (logAxis && !isNaN(tickMarks.step)) {
                    var arr = [];
                    for (var i = 0; i < tickMarks.offsets.length; i += tickMarks.step)
                        arr.push(tickMarks.offsets[i]);

                    tickMarks.offsets = arr;
                }
            }
            ...
            return this._renderAxis(!swapXY, isMirror, axisSettings, rect, chartRect, mu, logAxis, valuesOnTicks, itemsInfo, isMeasure, duration);
        },
        ...
        _generateOffsets: function (min, max, axisSize, padding, interval, baseInterval, isTicksMode, tickPadding, isValue, flip) {
            ...
            if (count == 0) {
                if (isValue || isTicksMode)
                    offsets.push({ offset: padding.left + paddedSize / 2, value: min / factor });
                else
                    offsets.push({ offset: 0, value: min / factor });
            ...
            for (var i = min; i <= max; i += baseInterval, offset += wInt)
                offsets.push({ offset: offset, value: i / factor });
        ...
        _buildStats: function (rect) {
            ...
            for (var gidx = 0; gidx < this.seriesGroups.length; gidx++) {
                ...
                var xAxisStats = this._getXAxisStats(gidx, xAxis, (group.orientation == 'vertical') ? rect.width : rect.height);
        ...
        _mergeCommonValueAxisStats: function () {
            ...
            for (var i = 0; i < this.seriesGroups.length; i++) {
                ...
                if (isNaN(common.gmin) || common.gmin > stats.gmin)
                    common.gmin = stats.gmin;

                if (isNaN(common.gmax) || common.gmax < stats.gmax)
                    common.gmax = stats.gmax;

                if (isNaN(common.gsumP) || common.gsumP < stats.gsumP)
                    common.gsumP = stats.gsumP;

                if (isNaN(common.gsumN) || common.gsumN < stats.gsumN)
                    common.gsumN = stats.gsumN;

                if (isNaN(common.logBase) || common.logBase > stats.logBase)
                    common.logBase = stats.logBase;

                if (isNaN(common.minPercent) || common.minPercent > stats.minPercent)
        ...
        _calcOutputGroupStats: function (context) {
            ...
            var mn = logAxis ? gmin : $.jqx._rnd(isStacked && !isWaterfall ? gsumN : gmin, mu, false);
            var mx = logAxis ? gmax : $.jqx._rnd(isStacked && !isWaterfall ? gsumP : gmax, mu, true);
        ...
        _animatePieSlices: function (ctx, percent) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                if (showLabels) {
                    ...
                    var labelRadius = labelsSettings.radius || item.outerRadius + Math.max(sz.width, sz.height);
                    ...
                    if (labelRadius > item.outerRadius + 5 && labelLinesEnabled != false)
                        item.labelArrowPath = this._updateLebelArrowPath(item.labelArrowPath, cx, cy, labelRadius, item.outerRadius, radMid, labelsSettings.linesAngles != false, colors, settings);
                }
                ...
                if (group.polar || group.spider) {
                    ...
                    if (!isXAxis) {
                        ...
                        if (group.spider) {
                            ...
                            this._columnAsPieSlice(elements, 0, rect, polarAxisCoords, columnRect);
                            bandElement = elements[0];
                        }
                    }
                }
                else {
        ...
        _getColumnVOffsets: function (renderData, groupIndex, seriesCtx, itemIndex, isStacked, percent) {
            ...
            var columnsTopWidthPercent = group.columnsTopWidthPercent;
            if (isNaN(columnsTopWidthPercent) || columnsTopWidthPercent > 100)
                columnsTopWidthPercent = 100;
            if (columnsTopWidthPercent < 0)
                columnsTopWidthPercent = 0;

            var columnsBottomWidthPercent = group.columnsBottomWidthPercent;
            if (isNaN(columnsBottomWidthPercent) || columnsBottomWidthPercent > 100)
                columnsBottomWidthPercent = 100;
            if (columnsBottomWidthPercent < 0)
        ...
        _columnAsPieSlice: function (elements, elementIndex, plotRect, polarAxisCoords, columnRect) {
            ...
            if (elements && !isNaN(elementIndex)) {
        ...
        _animR: function (ctx, percent) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                if (symbolType == 'circle') {
                    ...
                    item.element = element = this._drawSymbol(
                        symbolType,
                        x,
                        y,
                        item.fill,
        ...
        _showToolTip: function (x, y, gidx, sidx, iidx) {
            ...
            if (!isCrossHairsOnly && self.showToolTips != false) {
                ...
                self._createTooltip(coord, group, text, { css: cssToolTip, fill: toolTipFill, stroke: toolTipStroke, symbolSize: symbolSize });
            }
        },
        ...
        _createTooltip: function (position, group, content, style) {
            ...
            if (!divToolTip) {
                ...
                $(divToolTip).css('z-index', 9999999);
                ...
                $(arrowInnerDiv).css('z-index', 9999999 + 1);
                ...
                $(arrowInnerDiv).css('z-index', 9999999 + 2);
        ...
        _getAxisSettings: function (axis) {
            ...
            var gridLinesSettings = {
                visible: (axis.showGridLines != false && gridLinesProperties.visible != false),
            ...
            {
                visible: (axis.showTickMarks != false && tickMarksProperties.visible != false),
                    ...
                    if (symbol != 'none' || showLabels) {
                        ...
                        for (var i = 0; i < split.length; i++) {
                            ...
                            if (symbol != 'none') {
                                ...
                                var symbolElement = this._drawSymbol(symbol, point.x, point.y, itemColors.fillColorSymbol, settings.opacity, itemColors.lineColorSymbol, settings.opacity, 1, undefined, symbolSize);
        ...
        onmousemove: function (x, y) {
            ...
            if (g.type.indexOf('line') != -1 || g.type.indexOf('area') != -1) {
                ...
                if (this.showToolTips || this.enableCrosshairs) {
                    ...
                    this._pointMarker.element = this._drawSymbol(symbolType, x, y, colors.fillColorSymbolSelected, opacity, colors.lineColorSymbolSelected, opacity, 1, undefined, symbolSize);
        ...
        _getXAxisStats: function (groupIndex, xAxis, axisSize) {
            ...
            var group = this.seriesGroups[groupIndex];
            if (!isDateTime && (group.polar || group.spider)) {
                min = $.jqx._rnd(min, interval, false);
                max = $.jqx._rnd(max, interval, true);
            }

            return { min: min, max: max, dsRange: { min: minDS, max: maxDS }, filterRange: filterRange, useIndeces: useIndeces, isDateTime: isDateTime, isTimeUnit: isTimeUnit, dateTimeUnit: dateTimeUnit, interval: interval };
        },
        ...
        _getDTIntCnt: function (min, max, interval, dateTimeUnit) {
            ...
            while (curr.valueOf() < maxDate.valueOf()) {
                if (dateTimeUnit == 'millisecond')
                    ...
                    var currSave = curr.valueOf();

                    curr.setHours(curr.getHours() + interval);

                    if (currSave === curr.valueOf()) // DST FF bug
                        curr.setHours(curr.getHours() + interval + 1);
                }
                else if (dateTimeUnit == 'day')
        ...
        _castAsDate: function (value, dateFormat) {
            ...
            if (typeof (value) == 'string') {
                ...
                if ($.jqx.dataFormat) {
                    result = $.jqx.dataFormat.tryparsedate(value, undefined, false);
                    if (this._isDate(result))
                        return result;
                }
        ...
        _getBezierPoints: function (arr) {
            ...
            if (Math.abs(points[len - 2] - points[len - 4]) < 3 || Math.abs(points[len - 1] - points[len - 3]) < 3 || this._isVML) {
        ...
        */
        _renderXAxisRangeSelector: function (groupIndex, rect) {
            ...
            if (!this._isSelectorRefresh) {
                ...
                var element = $(div).appendTo(renderTo ? renderTo : this.host.find('.chartContainer'));
        ...
        _refreshSelector: function (groupIndex, axis, axisStats, renderTo, rect, swapXY) {
            ...
            xAxisSettings.rangeSelector = undefined;
            ...
            self.addHandler($(document), self._getEvent('mousemove') + '.' + self.element.id, self._onSliderMouseMove, { self: this, groupIndex: groupIndex, renderTo: renderTo, swapXY: swapXY });
            self.addHandler($(renderTo), self._getEvent('mousedown'), self._onSliderMouseDown, { self: this, groupIndex: groupIndex, renderTo: renderTo, swapXY: swapXY });
            self.addHandler($(renderTo), self._getEvent('mouseup'), self._onSliderMouseUp, { self: this, groupIndex: groupIndex, renderTo: renderTo, swapXY: swapXY });
        },

        _createSliderElements: function (groupIndex, renderTo, rect, selectorSettings) {
            renderTo.find('.slider').remove();

            var colorSelectedRange = selectorSettings.colorSelectedRange || 'blue';
            var colorUnselectedRange = selectorSettings.colorUnselectedRange || 'white';

            var div = $("<div class='slider' style='position: absolute;'></div>");
            div.css({ background: colorSelectedRange, opacity: 0.1, left: rect.x, top: rect.y, width: rect.width, height: rect.height });
            ...
            var divAreaDef = "<div class='slider' style='position: absolute;  background: " + colorUnselectedRange + "; opacity: 0.5;'></div>";
            var divBorderDef = "<div class='slider' style='position: absolute; background: grey; opacity: 0.5;'></div>";
            var divBarDef = "<div class='slider jqx-rc-all' style='position: absolute; background: white; border-style: solid; border-width: 1px; border-color: grey;'></div>";

            this._sliders[groupIndex] = {
                element: div,
                host: renderTo,
                fullRect: { x: div.coord().left, y: div.coord().top, width: rect.width, height: rect.height },
                rect: rect,
                left: $(divAreaDef),
                right: $(divAreaDef),
        ...
        _valueToOffset: function (groupIndex, value) {
            ...
            return slider.fullRect[sizeProp] * (axis.flip ? (1 - percent) : percent);
        },
        ...
        _offsetToValue: function (groupIndex, offset) {
            ...
            var denom = slider.fullRect[sizeProp];
            if (denom == 0)
                denom = 1;
        ...
        _onSliderMouseUp: function (event) {
            ...
            var minValue = self._offsetToValue(groupIndex, from - slider.fullRect[posProp]);
            var maxValue = self._offsetToValue(groupIndex, to - slider.fullRect[posProp]);
